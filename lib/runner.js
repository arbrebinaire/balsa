    ...
    run: (name) => {
        ...
        const run = async () => {
            ...
            accountsPath.forEach(dir => {
                ...
                if (fs.existsSync(path.join(cloud, dir, 'balsa', name))) {
                    found = path.join(cloud, dir, 'balsa', name)
                }
            });
            if (!found) {
            ...
            if (found) {
                if (shell.exec(`node ${path.join(found, 'index.js')}`).code === 0) {
                    console.log(chalk.green(`L'exécution de [${name}] a été un succès`))
                } else {
                    console.log(chalk.red(`Erreur: l'exécution de [${name}] ne s'est terminée normalement.`))
                }
            } else {
