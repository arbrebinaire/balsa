...
function getCommandlineStr(argv) {
...
module.exports = {
    run: (name, argv) => {
        ...
        const run = async () => {
            ...
            if (found) {
                shell.cd(found)
                shell.exec('npm install --no-shrinkwrap', {silent: true})
                console.log(getCommandlineStr(argv))
                if (shell.exec(`node ${path.join(found, 'index.js')} --color=always ${getCommandlineStr(argv)}`).code === 0) {
