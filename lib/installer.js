...
function getScriptsDir(){
    return path.join(balsaDir, 'balsa', 'scripts')
}
...
module.exports = {
    ...
    install: (prog, os) => {
        ...
        switch (os) {
            case 'ubuntu':
                switch (prog) {
                    case 'epson_image_scan':
                        executable = 'bash ' + path.join(getScriptsDir(), os, 'install', `${prog}.sh`)
                        break;
                    default:
                        console.log(chalk.yellow(`Programme d'installation inconnu: [${prog}]`))
                        break;
                }
                break;
        
            default:
                console.log(chalk.yellow(`Système d'exploitation inconnu: [${os}]`))
                break;
        }
        if(shell.exec(executable).code === 0){
            console.log(chalk.green("Le programme d'installation s'est terminé normalement."))
        } else {
            console.log(chalk.red("Erreur: le programme d'installation s'est terminé anormalement."))
        }
    }
  };
